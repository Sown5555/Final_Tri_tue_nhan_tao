...
# =========================
def load_cnn_model():
    # Sử dụng trực tiếp biến 'test1' của bạn
    if not os.path.exists(test1):
        print(f"⚠️ Không tìm thấy model CNN tại {test1}")
        return None
    try:
        ...
        print(f"❌ Lỗi nghiêm trọng khi tải model CNN: {e}")
        return None
...
def execute(image_data):
    ...
    if _core_cnn is None: return {"error": "Mô hình CNN chính chưa được tải."}

    # --- 1. Lấy xác suất từ tất cả các mô hình ---
    ...
    if not final_prediction:
        ...
        if len(top_candidates) == 1:
            ...
            if tie_breaker_scores:
                ...
                decision_method = "Số đông & Tổng xác suất (xử lý hòa)"
