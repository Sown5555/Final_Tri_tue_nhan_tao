...
def execute(image_data):
    ...
    def find_consensus(predictions_with_source):
        ...
        if count >= 3:
            return most_common_pred, f" ({count}/{len(predictions_with_source)} models)"
        if count == 2:
            models_that_voted = {p[0] for p in predictions_with_source if p[1] == most_common_pred}
            if models_that_voted == {'mlp', 'rf'}:
                return most_common_pred, "đặc biệt"
        return None, None
    top1_with_source = []
    if top3_cnn: top1_with_source.append(('0', top3_cnn[0][0]))
    if top3_cnn1: top1_with_source.append(('1', top3_cnn1[0][0]))
    if top3_cnn2: top1_with_source.append(('2', top3_cnn2[0][0]))
    if top3_cnn3: top1_with_source.append(('3', top3_cnn3[0][0]))
    final_prediction, method = find_consensus(top1_with_source)
    if final_prediction:
        decision_method = f"{method}"
    if not final_prediction:
        top2_with_source = []
        if len(top3_cnn) > 1: top2_with_source.append(('0', top3_cnn[1][0]))
        if len(top3_cnn1) > 1: top2_with_source.append(('1', top3_cnn1[1][0]))
        if len(top3_cnn2) > 1: top2_with_source.append(('2', top3_cnn2[1][0]))
        if len(top3_cnn3) > 1: top2_with_source.append(('3', top3_cnn3[1][0]))
        final_prediction, method = find_consensus(top2_with_source)
        if final_prediction:
            decision_method = f"{method}"
    if not final_prediction:
        top3_with_source = []
        if len(top3_cnn) > 2: top3_with_source.append(('0', top3_cnn[2][0]))
        if len(top3_cnn1) > 2: top3_with_source.append(('1', top3_cnn1[2][0]))
        if len(top3_cnn2) > 2: top3_with_source.append(('2', top3_cnn2[2][0]))
        if len(top3_cnn3) > 2: top3_with_source.append(('3', top3_cnn3[2][0]))
